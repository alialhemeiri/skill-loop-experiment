<!-- v2: Resolve conflicting values only through explicit amendment or supersession language; otherwise return null. -->
...
5. If operative provisions give conflicting values for a field, use a later addendum or amendment
   only when it explicitly replaces, varies, or supersedes the earlier term. Page order or recency
   alone does not resolve a conflict; return `null` when no controlling value can be determined.
