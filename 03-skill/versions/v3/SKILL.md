<!-- v2: Resolve repeated or conflicting values only through explicit document precedence, otherwise return null. -->
...
- When a field has repeated or conflicting values, follow only precedence established by the
  document: an amendment, addendum, or special condition explicitly modifying a term supersedes
  the term it modifies. If no applicable precedence resolves the conflict, return JSON `null`.
