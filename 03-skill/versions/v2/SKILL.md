<!-- v2: Resolve duplicate or conflicting field values by honoring explicit amendments and otherwise returning null when the operative value is unclear. -->
...
- If the document gives conflicting candidates for a field, an explicit amendment, addendum, or
  replacement term that supersedes the original controls. Otherwise use the value clearly tied to
  the operative tenancy terms; if the conflict cannot be resolved from the document, return `null`.
